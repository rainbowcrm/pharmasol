...
public class NextUpGenerator {
	...
	public static String getNextNumber(String program, ProductContext context , Division division  , Region  region, Date date) {
		...
		String part1 = makeCompString(config.getComponent1(), division, region, company, seq,date,config.getDateFormat());
		if (!Utils.isNull(part1)) {
			ansString.append(part1);
		}
		String part2 = makeCompString(config.getComponent2(), division,region,company, seq,date,config.getDateFormat());
		if (!Utils.isNull(part2)) {
			ansString.append(part2);
		}
		String part3 = makeCompString(config.getComponent3(), division,region, company, seq,date,config.getDateFormat());
		if (!Utils.isNull(part3)) {
			ansString.append(part3);
		}
		String part4 = makeCompString(config.getComponent4(), division,region, company, seq,date,config.getDateFormat());
	...
	private static String makeCompString(NextUpConfig.NextUpComponent component , Division division, Region region,
										 Company company, int sequence , Date date, String format) {
		StringBuffer buff = new StringBuffer();
		...
		}else if(component.getFieldType().equals(REGCODE)) {
