...
public class TargetAnalyzerController extends AbstractGeneralController {
    ...
    public PageResult read(ModelObject object) {
        ...
        if(analyzer.getBasedOn().equalsIgnoreCase("AGS")) {
            BarChartData barChartData = targetAnalyzeService.getAgentSaleBarChart(target, getProductContext());
            analyzer.setTargetData(barChartData);
        }

        if(analyzer.getBasedOn().equalsIgnoreCase("ITS")) {
            BarChartData barChartData = targetAnalyzeService.getItemSaleBarChart(target, getProductContext());
            analyzer.setTargetData(barChartData);
        }
